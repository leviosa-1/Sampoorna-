...
if($conn):
    ...
    if($_POST['s']):
        ...
        if($_SESSION["Ename"]!=='')
        {$sql5="insert into eventt(ClubID,Ename) values('{$_SESSION['lastid']}','{$_SESSION['Ename']}')";
    ...
    if($check1)
     {if(mysqli_num_rows($check1)>0)
      {$r1=mysqli_fetch_assoc($check1);
       $OFID=$r1['FID'];
       $sql7="INSERT INTO Event_organizer VALUES ('{$_SESSION['lastid']}',101,$OFID)";
       ...
       $sql9="INSERT INTO Event_organizer VALUES ('{$_SESSION['lastid']}',101,'{$_SESSION['lastfid']}')";
...
$COID=$r1['COID'];
$sql11="INSERT INTO Event_coordination VALUES ('{$_SESSION['lastid']}',101,$COID,'{$_SESSION['Cotype']}')";
...
$sql13="INSERT INTO Event_coordination VALUES ('{$_SESSION['lastid']}',101,'{$_SESSION['lastCoid']}','{$_SESSION['Cotype']}')";
...
if($_SESSION["Speaker"]!==''){
$sql16="select max(SID) as MAXID from Speaker group by Speaker.ClubID,Speaker.EventID having Speaker.ClubID='{$_SESSION['lastid']}'";
...
{if(mysqli_num_rows($check4)>0)
  ...
  $MAXID=$r1['MAXID'];
  $NEWSID=$MAXID+1;
  $sql17="INSERT INTO Speaker VALUES ('{$_SESSION['lastid']}',101,$NEWSID,'{$_SESSION['Speaker']}','{$_SESSION['SpeakerDesc']}')";
  $stmt=$conn->prepare($sql17);
  $stmt->execute();}
  else
{$sql18="INSERT INTO Speaker(ClubID,EventID,Sname,Sdescription) VALUES ('{$_SESSION['lastid']}',101,'{$_SESSION['Speaker']}','{$_SESSION['SpeakerDesc']}')";
...
$sql19="INSERT INTO Event_description VALUES ('{$_SESSION['lastid']}',101,$EDID,'{$_SESSION['EDesc_head']}','{$_SESSION['EDescription']}')";
...
$new_name="C".$_SESSION['lastid']."_E101_Sno".$SNO.".".$extension;
$folder="./image/".$new_name;
$sql20="INSERT INTO Event_pics VALUES ($SNO,'{$_SESSION['lastid']}',101,'$new_name')";
